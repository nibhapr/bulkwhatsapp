       ...
       if($cek){
        ...
        switch ($type) {
            ...
            case 'document';
                ...
                $arr = explode('.',$request->document);
                $ext = end($arr);
                $allowext = ['pdf','xls','xlsx','doc','docx'];
                if(!in_array($ext,$allowext)){
                    return redirect(route('autoreply'))->with('alert',[
                        'type' => 'danger',
                        'msg' => 'Only extension pdf,xls,xlsx,doc and docx allowed!'
                    ]);
                }
                $reply = [
                   "document" => ["url" => $request->document],
                ];
                break;
            // By Morehere
            case 'button':
        ...
        if($request->ajax()){
            switch ($type) {
                ...
                case 'image' :
                    ...
                    break;
                case 'document' :
                    return view('ajax.autoreply.formdocument')->render();
                    break;
                case 'button' :
